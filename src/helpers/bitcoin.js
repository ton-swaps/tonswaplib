...
class Bitcoin {
  ...
  fetchBalance(address) {
    // 10 seconds cache
    // query requests
    return request.get(`${this.root}/address/${address}/balance`,
      {
  ...
  fetchUnspents(address) {
    // 10 seconds cache
    // query requests
    return request.get(`${this.root}/address/${address}/?unspent=true`,
      {
        cacheResponse: 10*1000,
        queryResponse: true,
      }
  ...
  */
  ...
  async sendTransaction({ account, to, value }, handleTransactionHash) {
    ...
    unspents.forEach(({ mintTxid, vout }) => tx.addInput(mintTxid, vout, 0xfffffffe))
